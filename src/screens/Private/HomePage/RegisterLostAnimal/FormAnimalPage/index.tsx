...
const FormAnimal: React.FC = () => {
  ...
  console.log(params);
  const [position, setPosition] = useState<IParams>(params);
  ...
  async function handleAnimal(data: IData) {
    try {
      ...
      await api
        ...
        .then(async (response) => {
          const { aniID } = response.data;

          console.log(position.params.position.longitude);

          await api
            .post("/search-animals", {
              aniID,
              seaDescription: data.descPerdido,
              seaEventDate: new Date(Date.now()),
              seaLatitude: position.params.position.latitude,
              seaLongitude: position.params.position.longitude,
              seaReward: data.recompensa,
            })
            .then((response) => {
    ...
    <Container contentContainerStyle={{ padding: 25 }}>
      <Form ref={formRef} onSubmit={handleAnimal}>
        ...
        <Container>
          ...
          <TitleInput>Como Perdeu?</TitleInput>
          <Input
            name="descPerdido"
            style={{ height: 120 }}
            placeholder="Descrição como você perdeu seu petzin"
            multiline
          />

          <TitleInput>Recompensa?</TitleInput>
          <Input
            name="recompensa"
            style={{ height: 120 }}
            placeholder="Valor da recomepensa"
          />
        </Container>
